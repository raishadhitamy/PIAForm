...
Public Class MatManagerAgreement
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Dim command As New SqlCommand("select f.DocumentID, f.Part_Number, f.Quantity_Physical, f.Quantity_Book, f.Total_Value, f.Initiator_Name from frompia as f join WebRegistration as w on f.Initiator_Name = w.Name where w.KPK_MatMgr = (select KPK from WebRegistration where Name = '" & Session("name") & "') and Approval = (select Name from WebRegistration where KPK = any (select KPK_EngMgr from WebRegistration where Name = any (select Name from WebRegistration where KPK_MatMgr = any (Select KPK from WebRegistration where Name = '" & Session("name") & "'))))", Connect)
    ...
    Protected Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        Response.Redirect("FullDataMatManager.aspx")
    End Sub
    ...
    Protected Sub MatBtnUpdate_Click(sender As Object, e As EventArgs) Handles MatBtnUpdate.Click
        Connect.Open()
        For Each row As GridViewRow In MatDisplayDatabase.Rows
