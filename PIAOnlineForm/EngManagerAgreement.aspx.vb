...
Public Class EngManagerAgreement
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Dim command As New SqlCommand("select f.DocumentID, f.Plant, f.Source, f.Quantity_Physical, f.Quantity_Book, f.Total_Value, f.Initiator_Name from frompia as f join WebRegistration as w on f.Initiator_Name = w.Name where w.KPK_EngMgr = (select KPK from WebRegistration where Name = '" & Session("name") & "') and Approval = (select Name from WebRegistration where KPK = any (select KPK_Manager from WebRegistration where Name = any (select Name from WebRegistration where KPK_EngMgr = any (Select KPK from WebRegistration where Name = '" & Session("name") & "'))))", Connect)
    ...
    Protected Sub EngBtnUpdate_Click(sender As Object, e As EventArgs) Handles EngBtnUpdate.Click
        Connect.Open()
        For Each row As GridViewRow In EngDisplayDatabase.Rows
